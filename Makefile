...
dev: install
	@echo "Killing anything on ports 8000/4200..."
...
serve: build-frontend
	cd $(BACKEND_DIR) && $(PYTHON) -m uvicorn app:app --host 0.0.0.0 --port 8000
...
help:
	...
	@echo "  make dev              Install deps + launch backend (:8000) and frontend (:4200)"
	@echo "  make dev-backend      Start FastAPI backend only on :8000"
	@echo "  make dev-frontend     Start Angular dev server only on :4200"
	@echo "  make build            Build Angular frontend for production"
	@echo "  make serve            Build frontend + start production server on :8000"
